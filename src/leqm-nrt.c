...
printAudioFrameInfo (const AVCodecContext * codecContext,
    ...
    if (codecContext->channels > AV_NUM_DATA_POINTERS
        ...
        printf ("frame->data. Therefore, to access the audio data, you need to use\n");
        printf ("frame->extended_data to access the audio data. It's planar, so\n");
        printf ("each channel is in a different element. That is:\n");
        printf ("  frame->extended_data[0] has the data for channel 1\n");
        printf ("  frame->extended_data[1] has the data for channel 2\n");
        printf ("  etc.\n");
    } else {
        printf ("Either the audio data is not planar, or there is enough room in\n");
        printf ("frame->data to store all the channels, so you can either use\n");
    ...
    printf (" the red, green, and blue channels separately in different arrays.\n");
}
#endif
...
main (int argc, const char **argv) {
    ...
    printf ("leqm-nrt  Copyright (C) 2011-2013, 2017-2020 Luca Trisciani\n");
    printf ("This program comes with ABSOLUTELY NO WARRANTY,\n");
    printf ("for details on command line parameters see --help\n");
    printf ("First argument is the audio file to be measured.\n");
    printf ("Other parameters can follow in free order.\n");
    printf ("This is free software, and you are welcome to redistribute it\n");
    printf ("under the GPL v3 licence.\n");
    printf ("Program will use 1 + %d slave threads.\n", numCPU);
    //SndfileHandle file;
...
equalinterval3 (double freqsamples[], double freqresp_db[],
    ...
    for (int ieq = 0, i = 0; ieq < points; ieq++) {
        freq = ieq * pass;
        eqfreqsamples[ieq] = freq;
        if (freq < freqsamples[0]) {  /* If frequency is less than first sampling point of M definition, slope of first and second point is used */ // this has a lot of influence on final Leq(M) value
